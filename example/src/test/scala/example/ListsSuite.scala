 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum should throw a NoSuchElementException when list argument is empty") {
    val caughtException = intercept[NoSuchElementException] {
      sum(List())
    }
    assert(caughtException.getMessage == "Can't sum empty list")
  }

  test("sum should be positive when larger positive number is added to smaller negative number") {
    assert(sum(List(3, -2)) === 1)
  }

  test("sum should be positive when larger negative number is added to smaller positive number") {
    assert(sum(List(3, -5)) === -2)
  }

  test("sum should be zero when multiple zeroes are in list") {
  ...
  test("max of zero is zero") {
    assert(max(List(0)) === 0)
  }

  test("max of negative numbers is largest negative number") {
    assert(max(List(-3, -2, -4)) === -2)
  }

  test("max of negative and positive numbers is largest positive number") {
    assert(max(List(-3, 0, 5)) === 5)
  }
  ...
  test("max of empty list throws NoSuchElementException") {
    ...
    assert(caughtException.getMessage == "Can't max empty list")
  }
